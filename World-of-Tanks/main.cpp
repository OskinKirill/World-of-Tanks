...
public:
		...
		for (int i = y / 64; i < (y + h) / 64; i++)//проходимся по элементам карты
			for (int j = x / 64; j < (x + w) / 64; j++)
			{
				if (TileMap[i][j] == '0') //если элемент тайлик земли
				{
					...
					// if (Dx < 0) { x = j * 32 + 32; dx = 0; cout « "ла" }// с левым краем карты
					/*if (TileMap[i][j] == 's') {
			...
			if (name == "EasyEnemy") {//для персонажа с таким именем логика будет такой
				...
				if (life) {//проверяем, жив ли герой
					...
					case 0: {//состояние идти вправо
						...
						if (CurrentFrame > 3) CurrentFrame -= 3;
						sprite.setRotation(90);
						sprite.setTextureRect(IntRect(0, 0, 81, 81));
						break;
					}
	...
	public:
		...
		void update(float time)
		{
			...
			if (life) { // если пуля жива
				...
				sprite.setPosition(x + w * 1.5, y + h 1.5); //задается позицию пули
			}
		}
	};
	...
	int main()
	{
		...
		sf::RenderWindow window(sf::VideoMode(1280, 960, desktop.bitsPerPixel), "World of Tanks");

		Image map_image;//объект изображения для карты
		map_image.loadFromFile("images/New_map.png");//загружаем файл для карты
		...
		while (window.isOpen())
		{
			...
			clock.restart();
			p.life - &nbspДанный веб - сайт выставлен на..p.life
				time = time / 800;
			...
			for (int i = 0; i < HEIGHT_MAP; i++)
				...
				{
					if (TileMap[i][j] == ' ') s_map.setTextureRect(IntRect(192, 0, 64, 64)); // черный пол
					if (TileMap[i][j] == 's') s_map.setTextureRect(IntRect(0, 0, 64, 64)); // кирпичная стена
					if ((TileMap[i][j] == '0')) s_map.setTextureRect(IntRect(64, 0, 64, 64)); // железная
