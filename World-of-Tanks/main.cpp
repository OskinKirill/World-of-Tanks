...
int main()
{
	...
	while (window.isOpen())
	{
		...
		//Проверка пересечения врагов и игрока
		/*if (p.life == true)//если игрок жив
		for (it = enemies.begin(); it != enemies.end(); it++)	//проходим по эл-там списка врагов
		{
		if ((*it)->getRect().intersects(p.getRect()))	//если прямоугольник спрайта врага //пересекается с игроком
		{
		...
		for (int i = 0; i < HEIGHT_MAP; i++)
			...
			{
				if (TileMap[i][j] == ' ') s_map.setTextureRect(IntRect(96, 0, 32, 32)); // черный пол
				if (TileMap[i][j] == 's') s_map.setTextureRect(IntRect(0, 0, 32, 32)); // кирпичная стена
				if ((TileMap[i][j] == '0')) s_map.setTextureRect(IntRect(32, 0, 32, 32)); // железная
